...
class CohereMilvusRetriever:
    """Hybrid Milvus retriever with dense + sparse search and cross-encoder re-ranking"""
    
    def __init__(self, collection_name: str = "document_embeddings", 
                 embeddings=None, host: str = "localhost", port: str = "19530"):
    ...
    def add_documents(self, documents: List[Document]):
        """Add documents to the vector store and fit BM25"""
        if not documents:
    ...
    def retrieve(self, query: str, top_k: int = 5, rerank_top_k: int = 20) -> List[Dict]:
        ...
        if not self.embeddings:
            logger.error("No embeddings model provided")
            return []
    ...
    def _cross_encoder_rerank(self, query: str, candidate_docs: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Cross-encoder re-ranking completed. Top scores: {[doc['similarity'] for doc in candidate_docs[:3]]}")
            return candidate_docs
